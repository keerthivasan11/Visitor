...
public interface VisitorHistoryRepository extends JpaRepository<VisitorHistory, Long> {
        ...
        @Modifying
        @Query("UPDATE VisitorHistory v SET v.status = :status WHERE v.tenant.id = :tenantId")
        void updateStatusByTenantId(@Param("tenantId") Long tenantId, @Param("status") VisitStatus status);
