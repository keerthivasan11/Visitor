...
public class SuperAdminController {
    ...
    @GetMapping("/reports/visitors/export")
    public ResponseEntity<byte[]> exportVisitorReport(
            @RequestParam(required = false) // @DateTimeFormat(iso = DateTimeFormat.ISO.DATE)
            LocalDate startDate,
            @RequestParam(required = false) // @DateTimeFormat(iso = DateTimeFormat.ISO.DATE)
            LocalDate endDate,
            @RequestParam(required = false) Long tenantId) throws Exception {
        byte[] excel = reportService.exportVisitorReport(
    ...
    @GetMapping("/reports/vehicles/export")
    public ResponseEntity<byte[]> exportVehicleReport(
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate startDate,
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate endDate,
            @RequestParam(required = false) Long tenantId) throws Exception {

        byte[] excel = reportService.exportVehicleReport(
                startDate, endDate, tenantId);
    ...
    @GetMapping("/reports/staff/export")
    public ResponseEntity<byte[]> exportStaffReport(
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate startDate,
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate endDate)
            throws Exception {
        byte[] excel = reportService.exportStaffReport(
                startDate, endDate);
        return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_DISPOSITION,
                        "attachment; filename=staff_report.xlsx")
                .body(excel);
    }
}
